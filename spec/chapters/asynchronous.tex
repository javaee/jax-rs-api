...
Overloaded versions of the method \code{suspend} in \ExecutionContext\ also accept a timeout value and a unit, thus providing the same functionality as the \Suspend\ annotation. 

\section{EJB Resource Classes}
\label{async_ejbs}

As stated in Section~\ref{javaee}, JAX-RS implementations in products that
...
the response is produced by calling \code{resume} on \code{ExecutionContext}. Hence,
