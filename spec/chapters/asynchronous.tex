...
Resource classes that support asynchronous processing must inject an instance of \code{ExecutionContext} in order to suspend and resume connections. In the example above, the method \code{longRunningOp} is called upon receiving a \code{GET} request. Rather than producing a response immediately, this method: (i) calls \code{suspend} on the injected \code{ExecutionContext}, (ii) forks a (non-request) thread to execute a long running operation and (iii) returns immediately. Once the execution of the long running operation is complete, the connection is resumed and the response returned by calling the method \code{resume} on \code{ExecutionContext}.

\subsection{Suspend Annotation}
\label{suspend_annotation}

An alternative to calling \code{ctx.suspend()} as the first step after entering an asynchronous method is to annotate the method with \Suspend. Thus, the \code{longRunningOp} method above is equivalent to:
...
JAX-RS implementations are REQUIRED to generate a \WebApplicationException\ with a service unavailable error response (HTTP 503 status) if the timeout value is reached and no fallback response is set in \ExecutionContext. The exception MUST be processed as described in section~\ref{method_exc}. If a fallback response is set in \ExecutionContext, via the method \code{setResponse}, JAX-RS implementations are REQUIRED to return the fallback response without generating a \WebApplicationException\ when the timeout is reached.

Overloaded versions of the method \code{suspend} in \ExecutionContext\ also accept a timeout value and a unit, thus providing the same functionality as the \Suspend\ annotation. Note that it is forbidden to call \code{suspend} more than once or to call it from within a method that is already annotated with \Suspend.
...
the response is produced by calling \code{resume} on the \code{ExecutionContext}. Hence,
