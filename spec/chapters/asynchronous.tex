...
It is also possible to register callbacks on an instance of \code{AsyncResponse} in order to listen for processing completion (\code{CompletionCallback}) and connection termination (\code{ConnectionCallback}) events. See Javadoc for \code{AsyncResponse} for more information on how to register these callbacks. Note that support \code{ConnectionCallback} is OPTIONAL.
