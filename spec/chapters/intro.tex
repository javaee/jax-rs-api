...
\item[Filter] A class that implements \code{RequestFilter}, \code{PreMatchRequestFilter} or \code{ResponseFilter} (or any combination thereof) and is registered as a provider.
\item[Entity Interceptor] A class that implements \code{ReaderInterceptor} or \code{WriterInterceptor} (or both) and is registered as a provider.
