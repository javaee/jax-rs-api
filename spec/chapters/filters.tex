...
As stated in Section \ref{introduction_filters}, a global \ContainerRequestFilter\ is executed before resource matching unless annotated with \PostMatching. A global filter that injects \code{ResourceInfo}, and generally depends on resource information for its execution, must be annotated with \PostMatching.
...
The following example defines a dynamic binder that binds the filter \code{LoggingFilter} ---assumed not globally bound for the purpose of this example--- with all the resource methods in \code{MyResourceClass} that are annotated with \code{@GET}.
...
The method \code{getBoundProvider} can return either an instance or a class. The parameter \code{T} in the type \code{DynamicBinder<T>} determines the return type. Returning a class, such as \code{LoggingFilter.class} in the example above, delegates the responsibility of instantiation (and injection) to the \jaxrs\ implementation.
