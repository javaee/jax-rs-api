...
\hline
\hline
\code{BindingPriority} & \raggedright Type & \raggedright  Specifies a binding priority for a filter or interceptor. Binding priorities are represented by integer numbers. The default binding priority is 500 (\code{BindingPriority.USER}). \tabularnewline
\hline
...
\hline
\code{BeanParam} & \raggedright  Parameter, field or method & \raggedright Can be used to inject a user-defined bean whose fields and properties may be annotated with JAX-RS param annotations.\tabularnewline\hline
\hline
...
\tabularnewline\hline
\code{ParamsConverter.Lazy} & \raggedright Type & \raggedright Indicates that a conversion of a default value delegated to a \code{ParamConverter} SHOULD occur only when the value is actually requested.
\tabularnewline\hline
\end{longtable}
%\end{center}
