...
\begin{itemize}
\item Add $R(T_Z)$ where $T_Z$ is the URI path template specified for the class $Z$.
\end{itemize}
...
\item Let $C'$ to be the ordered set of root resource classes that correspond to the elements in $E$. By definition, $C'$ must be a non-empty subset of $C$ at this point.

\end{enumerate}

\item \label{find_object} Obtain a candidate object and a subset candidate methods for the request:
...
\begin{enumerate}
\item \label{loop_root} Let $Z'$ be the next class in the ordered set $C'$. If all classes in $C'$ have been processed, the algorithm terminates by generating a \WebAppExc\ with a not found response (HTTP 404 status) and no entity. 

\item Let $O$ be an instance of class $Z'$.
...
\item For class $Z'$ add regular expressions to $E$ for each sub-resource method and locator as follows:
\begin{enumerate}
...
\item If $E$ is empty and there are {\em more} root resource classes to process in $C'$, go to step \ref{loop_root}. If $E$ is empty and there are {\em no more} classes in $C'$, then no matching resource can be found and the algorithm terminates by generating a \WebAppExc\ with a not found response (HTTP 404 status) and no entity. 
...
\begin{description}
\item[Input] $O=\mbox{instance of resource class matched}, M=\mbox{candidate resource methods of $O$}$
...
\item \label{dispatch_method} The request is dispatched to the first resource method $D$ in the set $M$\footnote{Step \ref{filter_methods} ensures the set contains at least one member.}. If after sorting, there is more than one maximum element in $M$, implementations SHOULD report a warning and select one of these methods in an implementation dependent manner.
\end{enumerate}
...
\begin{description}
\item[Step 1] Match root resource class using $R(\mbox{widgets}) = \mbox{widgets(/.*)?}$ and $R(\mbox{widget}) = \mbox{widget(/.*)?}$
\begin{description}
...
\end{description}
\item[Step 2] Match resource class instance using $R(\{\mbox{id}\}) = \mbox{([\^{ }/]+?)(/.*)?}$
\begin{description}
\item[Input] $U = \mbox{/1}$ and $C' = \{\mbox{WidgetsResource}\}$
\item[Output] $O = \mbox{WidgetResource instance}$ and $M = \{\mbox{findWidget}\}$
\end{description}
\item[Step 3] Match resource class method
\begin{description}
\item[Input] $O = \mbox{WidgetResource instance}$ and $M = \{\mbox{findWidget}\}$
