...
\item[Output] $O=\mbox{instance of resource class matched}, m=\mbox{resource method matched from $M$}$
\end{description}
...
\item \label{dispatch_method} The request is dispatched to the first resource method $m$ in the set $M$\footnote{Step \ref{filter_methods} ensures the set contains at least one member.}. If after sorting, there is more than one maximum element in $M$, implementations SHOULD report a warning and select one of these methods in an implementation dependent manner.
\end{enumerate}
...
\item[Output] $U = \mbox{/1}$ and $O = \mbox{WidgetsResource instance}$
\end{description}
\item[Step 2] Match resource class instance using $R(\{\mbox{id}\}) = \mbox{([\^{ }/]+?)(/.*)?}$
\begin{description}
\item[Input] $U = \mbox{/1}$ and $O = \mbox{WidgetsResource instance}$
